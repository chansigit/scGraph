...
from .utils_handler import calculate_trimmed_means, compute_classwise_distances
...
class scGraph:
    ...
    def process_batches(self):
        ...
        for BATCH_ in tqdm(self.adata.obs[self.batch_key].unique()):
            ...
            # # Count unique cell types (excluding ignored ones)
            # valid_celltypes = set(adata_batch.obs[self.label_key].unique()) - set(self.ignore_celltype)
            # if len(valid_celltypes) < 2:
            #     print(f"Skipped batch {BATCH_}, fewer than 2 valid cell types")
            #     continue
            ...
            pca_pairdist = compute_classwise_distances(centroids_pca)
    ...
    def adata_concensus(self, obsm):
        _centroid = calculate_trimmed_means(
            np.array(self.adata.obsm[obsm]),
            self.adata.obs[self.label_key],
            trim_proportion=self.trim_rate,
            ignore_=self.ignore_celltype,
        )
        _pairdist = compute_classwise_distances(_centroid)
    ...
    def main(self, _obsm_list=None):
        ...
        if _obsm_list is None:
            _obsm_list = self.obsm_keys if self.obsm_keys is not None else sorted(list(self.adata.obsm))
...
def main():
    """Command-line interface for scGraph evaluation."""
    import argparse
    parser = argparse.ArgumentParser(description="scgraph metric calculation")
    ...
    parser.add_argument("--obsm_keys", type=str, nargs="+", default=None,
                        help="One or more obsm keys to evaluate (e.g. X_umap X_scVI). "
                             "If not set, all embeddings in adata.obsm are evaluated.")
